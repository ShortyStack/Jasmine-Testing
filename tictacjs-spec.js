...
describe("Tic Tac JS", function () {
    ...
    it("should update the board with correct move", function () {
        board = ["X", "O", null, null, null, null, null, null, null];
        updateBoard();

        expect($("#cell-" + 1).text()).toBe("O");
    })

    it("should not fill in the board slots that are already occupied", function(){
        board = ["X", null, null, null, null, null, null, null, null];
        computerMove();
        var oldBoardPiece = board[0];
        expect(oldBoardPiece).not.toBe("O"); 
    })   
